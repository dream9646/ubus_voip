...
// 322 Ethernet_frame_performance_monitoring_history_data_upstream

static int 
meinfo_hw_322_get(struct me_t *me, unsigned char attr_mask[2])
{
	unsigned int i,ret;
	...
	for (i = 1; i <= meinfo_util_get_attr_total(me->classid); i++)
	{
		if (util_bitmap_get_value(attr_mask, 8*2, i - 1, 1))
		{
			...
			case 7:
				...
				break;
			case 8:
				meinfo_me_attr_update_pm(me, i, ethernet_frame_pm.crc_errored_packets);
				break;
			case 9:
...
struct meinfo_hardware_t meinfo_hw_322 = {
	.get_hw		= meinfo_hw_322_get,
};
