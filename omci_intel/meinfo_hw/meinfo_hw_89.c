...
// 89 Ethernet_performance_monitoring_history_data_2

static int 
meinfo_hw_89_get(struct me_t *me, unsigned char attr_mask[2])
{
	unsigned int i,ret;
	struct me_t *eth_pptp_uni_me;
	struct switch_ethernet_pm2_t ethernet_pm2;
	...
	memset( &ethernet_pm2, 0, sizeof(struct switch_ethernet_pm2_t) );

	if (switch_get_port_info_by_me(eth_pptp_uni_me, &sw_port_info) == 0) {
		if( (ret=switch_hw_g.eth_pm_data2(sw_port_info.logical_port_id, &ethernet_pm2)) !=0 ) {
	...
	for (i = 1; i <= meinfo_util_get_attr_total(me->classid); i++)
	{
		if (util_bitmap_get_value(attr_mask, 8*2, i - 1, 1))
		{
			...
			case 3:
				meinfo_me_attr_update_pm(me, i, ethernet_pm2.pppoe_filtered_frame_counter);
				break;
			default:
				continue;//do nothing
			}
...
struct meinfo_hardware_t meinfo_hw_89 = {
	.get_hw		= meinfo_hw_89_get,
};
