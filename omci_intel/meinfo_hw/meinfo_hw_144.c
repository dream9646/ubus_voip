...
//9.9.13 RTP_performance_monitoring_history_data
static int 
meinfo_hw_144_get(struct me_t *me, unsigned char attr_mask[2])
{
	int i;
	unsigned int pm_value;
	unsigned char omci_attr_mask[2];
	struct voip_hw_rtp_performance_monitoring_history_data_t rpmhd;
	...
	memset(&rpmhd, 0x00, sizeof(rpmhd));

	if (meinfo_hw_util_get_voip_param_by_port(me, &rpmhd, attr_mask, omci_attr_mask) < 0)
	{
	...
	for (i = 1; i <= meinfo_util_get_attr_total(me->classid); i++)
	{
		if (util_bitmap_get_value(omci_attr_mask, 8*2, i - 1, 1))
		{
			...
			case 3:
				pm_value = rpmhd.rtp_errors;
				break;
			case 4:
				pm_value = rpmhd.packet_loss;
				break;
			case 5:
				pm_value = rpmhd.maximum_jitter;
				break;
			case 6:
				pm_value = rpmhd.maximum_time_between_rtcp_packets;
				break;
			case 7:
				pm_value = rpmhd.buffer_underflows;
				break;
			case 8:
				pm_value = rpmhd.buffer_overflows;
				break;
			default:
				continue;//do nothing
			}
...
struct meinfo_hardware_t meinfo_hw_144 = {
	.get_hw		= meinfo_hw_144_get
};
