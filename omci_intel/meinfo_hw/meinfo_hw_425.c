...
int meinfo_hw_425_get(struct me_t *me, unsigned char attr_mask[2])
{
	unsigned int i,ret;
	struct me_t *parent_me=NULL;
	struct switch_ethernet_frame_pm_ud_long_t ethernet_frame_pm;
	unsigned long long crc_errored_packets = 0;
	...
	memset( &ethernet_frame_pm, 0, sizeof(struct switch_ethernet_frame_pm_ud_long_t) );
	...
	if(parent_classid==11 || parent_classid==47 || parent_classid==263 || parent_classid==329) {
		...
		if (dir) // upstream
		{
			if( (ret=switch_hw_g.eth_pm_data_upstream_long(sw_port_info.logical_port_id, &ethernet_frame_pm)) !=0 ) {
		...
		else // downstream
		{
			...
			if( (ret=switch_hw_g.eth_pm_data_downstream_long(switch_get_wan_logical_portid(), &ethernet_frame_pm)) !=0 ) {
				dbprintf(LOG_ERR, "ret = %d\n", ret);
				return -1;
			}
			crc_errored_packets = ethernet_frame_pm.crc_errored_packets;
			if( (ret=switch_hw_g.eth_pm_data_downstream_long(sw_port_info.logical_port_id, &ethernet_frame_pm)) !=0 ) {
				dbprintf(LOG_ERR, "ret = %d\n", ret);
				return -1;
			}
		}
		...
		list_for_each_entry(bridge_port_me, &miptr->me_instance_list, instance_node) {
			if (me_related(bridge_port_me, mac_bridge_me)) {
				if(switch_get_port_info_by_me(bridge_port_me, &sw_port_info)==0) {
					struct switch_ethernet_frame_pm_ud_long_t pm;
					memset(&pm, 0x00, sizeof(struct switch_ethernet_frame_pm_ud_long_t));
					if (dir) {
						switch_hw_g.eth_pm_data_upstream_long(sw_port_info.logical_port_id, &pm);
					} else {
						switch_hw_g.eth_pm_data_downstream_long(sw_port_info.logical_port_id, &ethernet_frame_pm);
					}
...
struct meinfo_hardware_t meinfo_hw_425 = {
	.get_hw		= meinfo_hw_425_get,
};
