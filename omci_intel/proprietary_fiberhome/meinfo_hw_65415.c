...
//9.99.13 WRI_IPT_config_data
static int 
meinfo_hw_65415_get(struct me_t *me, unsigned char attr_mask[2])
{
	...
	if ((ret = er_group_hw_voip_wrapper_wri_ipt_config_data(VOIP_HW_ACTION_GET, port_id, attr_mask, &wicd)) < 0)
	{
	...
	for (i = 1; i <= meinfo_util_get_attr_total(me->classid); i++)
	{
		...
		if (util_bitmap_get_value(omci_attr_mask, 8*2, i - 1, 1))
		{
			...
			case 2:
				...
				if (!list_empty(&wicd.ipt_data_list) && (entry_size = meinfo_util_attr_get_table_entry_byte_size(65415, 2)) > 0)
				{
					list_for_each_entry(pde_pos, &wicd.ipt_data_list, pos_data_node)
					{
...
struct meinfo_hardware_t meinfo_hw_fiberhome_65415 = {
	.get_hw		= meinfo_hw_65415_get
};
