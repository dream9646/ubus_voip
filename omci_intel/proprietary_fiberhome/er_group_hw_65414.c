...
// 65414 WRI_POS_config_data

//65414@1,65414@2
int
er_group_hw_wri_pos_config_data(int action, struct er_attr_group_instance_t *attr_inst, struct me_t *me, unsigned char attr_mask[2])
{
	...
	wpcd.item_number = (unsigned char) attr_inst->er_attr_instance[0].attr_value.data;
	...
	if ((table_header = (struct attr_table_header_t *) attr_inst->er_attr_instance[1].attr_value.ptr) != NULL) {
		list_for_each_entry(table_entry_pos, &table_header->entry_list, entry_node)
		{
			if (table_entry_pos->entry_data_ptr != NULL)
			{
				...
				pd_entry->pos_type = (unsigned short) util_bitmap_get_value(table_entry_pos->entry_data_ptr, 8*23, 168, 16);
				list_add_tail(&pd_entry->pos_data_node, &wpcd.pos_data_list);
			}
		}
	} else {
	...
	case ER_ATTR_GROUP_HW_OP_ADD:
		if ((ret = er_group_hw_voip_wrapper_wri_pos_config_data(VOIP_HW_ACTION_SET, port_id, attr_mask, &wpcd)) < 0)
		{
	...
	case ER_ATTR_GROUP_HW_OP_UPDATE:
		...
		//add
		if (er_group_hw_wri_pos_config_data(ER_ATTR_GROUP_HW_OP_ADD, current_attr_inst, me, attr_mask) < 0)
		{
	...
	case ER_ATTR_GROUP_HW_OP_DEL:
		...
		memset(&wpcd, 0x00, sizeof(wpcd));
		INIT_LIST_HEAD(&wpcd.pos_data_list);
		if ((ret = er_group_hw_voip_wrapper_wri_pos_config_data(VOIP_HW_ACTION_RELEASE, port_id, attr_mask, &wpcd)) < 0)
		{
			dbprintf(LOG_ERR, "ret=%d\n", ret);
			return -1;
		}
