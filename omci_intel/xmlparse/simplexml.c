...
int
simpleXmlInitializeParser(SimpleXmlParser parser, const char *sData, long nDataSize)
{
 ...
 */
int
initializeSimpleXmlParser(SimpleXmlParserState parser, const char *sData, long nDataSize)
{
...
parseOneTag(SimpleXmlParserState parser, SimpleXmlTagHandler parentHandler)
{
	...
	while (parser->nNextToken != TAG_END && parser->nNextToken != TAG_BEGIN_CLOSING) {
		/* read attributes */
		if (parser->nNextToken == ATTRIBUTE) {
			...
			handler((SimpleXmlParser) parser, ADD_ATTRIBUTE, szTagName,
				parser->szAttribute, getInternalSimpleXmlValueBufferContents(parser->vbNextToken));
			if (parser->nError != NO_ERROR) {
				free(szTagName);
				return FAIL;
			}
	...
	if (parser->nNextToken == TAG_BEGIN_CLOSING) {
		...
		if (strcmp(szTagName, getInternalSimpleXmlValueBufferContents(parser->vbNextToken)) != 0) {
...
readNextTagToken(SimpleXmlParserState parser)
{
	...
	if (peekInputChar(parser) == '/') {
		...
		if (getSimpleXmlValueBufferContents
		    (parser->vbNextToken, parser->szAttribute, parser->nAttributeBufferSize) == FAIL) {
			parser->nError = OUT_OF_MEMORY;
			return FAIL;
		}
		clearSimpleXmlValueBuffer(parser->vbNextToken);
...
readNextContentToken(SimpleXmlParserState parser)
{
	...
	if (peekInputChar(parser) == '<') {
		...
		if (peekInputChar(parser) == '/') {
			...
			skipInputChar(parser);
			while (peekInputCharAt(parser, 0) != '?' || peekInputCharAt(parser, 1) != '>') {
				/* read one character into next token value buffer */
				if (readChar(parser) == FAIL) {
					return FAIL;
				}
			...
			skipInputChar(parser);
			if (peekInputCharAt(parser, 0) == '-' && peekInputCharAt(parser, 1) == '-') {
				/* read comment */
				parser->nNextToken = COMMENT;
				skipInputChars(parser, 2);
				while (peekInputCharAt(parser, 0) != '-' ||
				       peekInputCharAt(parser, 1) != '-' || peekInputCharAt(parser, 2) != '>') {
					/* read one character into next token value buffer */
					if (readChar(parser) == FAIL) {
						return FAIL;
					}
				...
				skipInputChars(parser, 3);
				   ...
				   peekInputCharAt(parser, 5) == 'P' && peekInputCharAt(parser, 6) == 'E') {
			...
			while (peekInputChar(parser) > SPACE &&
			       peekInputChar(parser) != '/' && peekInputChar(parser) != '>') {
				/* read one character into next token value buffer */
				if (readChar(parser) == FAIL) {
					return FAIL;
				}
	...
	if (c == '\0' && parser->nError != NO_ERROR) {
		...
		if (peekInputCharAt(parser, 0) == '#') {
			...
			return addNextTokenCharValue(parser, (char) nCode);
			   ...
			   peekInputCharAt(parser, 2) == 'p' && peekInputCharAt(parser, 3) == ';') {
			/* &amp; -> & */
			...
			return addNextTokenCharValue(parser, '&');
			   ...
			   peekInputCharAt(parser, 3) == 's' && peekInputCharAt(parser, 4) == ';') {
			/* &apos; -> ' */
			...
			return addNextTokenCharValue(parser, '\'');
			   ...
			   peekInputCharAt(parser, 2) == 'o' &&
			   peekInputCharAt(parser, 3) == 't' && peekInputCharAt(parser, 4) == ';') {
			/* &quot; -> " */
			skipInputChars(parser, 5);
			return addNextTokenCharValue(parser, '"');
		} else if (peekInputCharAt(parser, 0) == 'l' &&
			   peekInputCharAt(parser, 1) == 't' && peekInputCharAt(parser, 2) == ';') {
			/* &lt; -> < */
			skipInputChars(parser, 3);
			return addNextTokenCharValue(parser, '<');
 ...
 */
int
getSimpleXmlValueBufferContents(SimpleXmlValueBuffer vb, char *szOutput, long nMaxLen)
{
