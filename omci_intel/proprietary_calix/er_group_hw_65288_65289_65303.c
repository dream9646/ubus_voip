 ...
 * Module  : er_group_hw
 * File    : er_group_hw_65288_65289_65303.c
 *
 ******************************************************************/
...
//9.99.39 65303 OamDmMepStats

// 65288@3 [0]
// 65289@1,2,4,5 [1-4]
// 65303@12 [5]
int 
er_group_hw_oam_dm_clear_counters(int action, struct er_attr_group_instance_t *attr_inst, struct me_t *me, unsigned char attr_mask[2])
{
	...
	case ER_ATTR_GROUP_HW_OP_ADD:
		if((cfm_config = cfm_mp_find(md_level, mepid, port_type, port_type_index)) != NULL) {
			...
			if (is_clear) {
				cfm_config->TxTimestampf = 0;
				cfm_config->TxTimestampb = 0;
				cfm_config->RxTimestampf = 0;
				cfm_config->RxTimestampb = 0;
				cfm_config->frame_delay_min = 0;
				cfm_config->frame_delay_max = 0;
				cfm_config->frame_delay_avg = 0;
				cfm_config->round_trip_min = 0;
				cfm_config->round_trip_max = 0;
				cfm_config->round_trip_avg = 0;
				cfm_config->response_time_min = 0;
				cfm_config->response_time_max = 0;
				cfm_config->response_time_avg = 0;
				memset(&cfm_config->cfm_pkt_pm[CFM_PDU_DMM], 0, sizeof(cfm_pm_counter_t));
				memset(&cfm_config->cfm_pkt_pm[CFM_PDU_DMR], 0, sizeof(cfm_pm_counter_t));
			}
		}
		break;
	...
	case ER_ATTR_GROUP_HW_OP_UPDATE:
		//delete
		if (er_group_hw_oam_dm_clear_counters(ER_ATTR_GROUP_HW_OP_DEL, attr_inst, me, attr_mask) < 0)
		{
		...
		//add
		if (er_group_hw_oam_dm_clear_counters(ER_ATTR_GROUP_HW_OP_ADD, current_attr_inst, me, attr_mask) < 0)
		{
