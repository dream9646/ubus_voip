 ...
 * File    : proprietary_calix_sw_download.c
 *
 ******************************************************************/
...
int
proprietary_calix_fill_resp_header_by_orig(struct proprietary_calix_sw_download_section_layout_t *req_msg, struct omcimsg_layout_t *resp_msg)
{
...
unsigned int
proprietary_calix_get_msg_priority(struct proprietary_calix_sw_download_section_layout_t *msg)
{
...
int 
proprietary_calix_download_section_handler(struct proprietary_calix_sw_download_section_layout_t *req_msg, struct omcimsg_layout_t *resp_msg)
{
	struct proprietary_calix_sw_download_section_t *download_section = (struct proprietary_calix_sw_download_section_t *) req_msg->msg_contents;
	long long current_window_offset, file_offset, data_size;
	int ret, i;
	extern struct download_info_t download_info;
	
#if 0	//detail valid check
	if (proprietary_calix_sw_download_section_header_get_entity_class_id(&req_msg) != 7) {	//sw image
	...
	current_window_offset = download_section->download_section_num * OMCIMSG_CALIX_DL_SECTION_CONTENTS_LEN;
	file_offset = download_info.last_window_end + current_window_offset;
	data_size =  download_info.image_size_in_bytes - file_offset;

	if( data_size > OMCIMSG_CALIX_DL_SECTION_CONTENTS_LEN) {
		data_size=OMCIMSG_CALIX_DL_SECTION_CONTENTS_LEN;
	} else if (data_size >=0) {
...
exit_download_section_handler:
	...
	if (proprietary_calix_sw_download_section_header_get_flag_ar(req_msg)) {
		...
		for(i=0; i <= download_section->download_section_num; i++ ) {
			if( download_info.section_map[i] != 1 ) {
				dbprintf(LOG_ERR, "window %d section %d error, offset=0x%X\n", download_info.last_window_end/((download_info.window_size_minus1+1)*OMCIMSG_CALIX_DL_SECTION_CONTENTS_LEN), i, download_info.last_window_end+OMCIMSG_CALIX_DL_SECTION_CONTENTS_LEN*i);
		...
		proprietary_calix_fill_resp_header_by_orig(req_msg, resp_msg);
...
#if defined CONFIG_SDK_PRX126 || defined CONFIG_SDK_PRX120 || defined CONFIG_SDK_PRX321
					switch_hw_fvt_g.sw_image_download_handle_window(download_info.instance_id, download_info.last_window_end/((download_info.window_size_minus1+1)*OMCIMSG_CALIX_DL_SECTION_CONTENTS_LEN), section_buf, current_window_offset + data_size);
#endif
				}
...
int
proprietary_calix_handle_download_section(struct proprietary_calix_sw_download_section_layout_t *req_msg)
{
	...
	proprietary_calix_download_section_handler(req_msg, resp_msg);
...
int
proprietary_calix_sw_download(char *recvbuf)
{
	static char crc_type = -1;	// use static variable to keep the crc_type calc result
	struct proprietary_calix_sw_download_section_layout_t *msg = (struct proprietary_calix_sw_download_section_layout_t *) recvbuf;
	...
	msg_crc = proprietary_calix_sw_download_section_trailer_get_crc(msg); //only for crc enabled and udp socket
	...
	switch(crc_type)
	{
		case ENV_CRC_TYPE_AUTO:
			...
			crc_be = ~(crc_be_update(crc_accum, (char *) msg, OMCIMSG_CALIX_DL_SECTION_BEFORE_CRC_LEN));
			crc_accum = 0xFFFFFFFF;
			crc_le = ~(crc_le_update(crc_accum, (char *) msg, OMCIMSG_CALIX_DL_SECTION_BEFORE_CRC_LEN));
			crc_le = htonl(crc_le);
		...
		case ENV_CRC_TYPE_BE:
			crc_accum = 0xFFFFFFFF;
			crc = ~(crc_be_update(crc_accum, (char *) msg, OMCIMSG_CALIX_DL_SECTION_BEFORE_CRC_LEN));
			break;
		case ENV_CRC_TYPE_LE:
			crc_accum = 0xFFFFFFFF;
			crc = ~(crc_le_update(crc_accum, (char *) msg, OMCIMSG_CALIX_DL_SECTION_BEFORE_CRC_LEN));
			crc = htonl(crc);
			break;
		...
		default:
			...
			crc_be = ~(crc_be_update(crc_accum, (char *) msg, OMCIMSG_CALIX_DL_SECTION_BEFORE_CRC_LEN));
			crc_accum = 0xFFFFFFFF;
			crc_le = ~(crc_le_update(crc_accum, (void *) msg, OMCIMSG_CALIX_DL_SECTION_BEFORE_CRC_LEN));
			crc_le = htonl(crc_le);
			if (crc_be == msg_crc)
			{
				crc = crc_be;
	...
	proprietary_calix_handle_download_section(msg);
