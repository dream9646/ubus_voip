...
//classid 65302 9.99.38 OamLmMepStats

static int 
meinfo_cb_65302_get(struct me_t *me, unsigned char attr_mask[2])
{
	...
	if (util_attr_mask_get_bit(attr_mask, 1)) { // MinNELoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 1, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 2)) { // MaxNELoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 2, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 3)) { // AvgNELoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 3, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 4)) { // CurNELoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 4, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 5)) { // MinFeLoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 5, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 6)) { // MaxFeLoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 6, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 7)) { // AvgFeLoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 7, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 8)) { // CurFeLoss
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 8, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 9)) { // RemoteMep
		struct attr_value_t attr;
		attr.data = 0;
		meinfo_me_attr_set(me, 9, &attr, 0);
	}
	if (util_attr_mask_get_bit(attr_mask, 10)) { // RemoteMac
		struct attr_value_t attr;
	...
	if (util_attr_mask_get_bit(attr_mask, 11)) { // ClearCounters
		struct attr_value_t attr;
		attr.data = 1;
		...
		if((cfm_config = cfm_mp_find(md_level, mep_id, port_type, port_idx)) != NULL) {
			if (!cfm_config->admin_state) {
				if (util_attr_mask_get_bit(attr_mask, 1)) { // MinNELoss
					...
					list_for_each_entry(entry, &cfm_config->cfm_recv_rmep_list, node) {
						if(memcmp(entry->rmep_mac, mac, IFHWADDRLEN)==0) {
							attr.data = (entry->eth_lm_single_rx_nloss<entry->eth_lm_single_tx_nloss)?entry->eth_lm_single_rx_nloss:entry->eth_lm_single_tx_nloss;
							meinfo_me_attr_set(me, 1, &attr, 0);
							break;
						}
					}
				...
				if (util_attr_mask_get_bit(attr_mask, 3)) { // AvgNELoss
					...
					list_for_each_entry(entry, &cfm_config->cfm_recv_rmep_list, node) {
						if(memcmp(entry->rmep_mac, mac, IFHWADDRLEN)==0) {
							attr.data = (entry->eth_lm_single_rx_nloss+entry->eth_lm_single_tx_nloss)/2;
							meinfo_me_attr_set(me, 3, &attr, 0);
							break;
						}
					}
				...
				if (util_attr_mask_get_bit(attr_mask, 4)) { // CurNELoss
					...
					list_for_each_entry(entry, &cfm_config->cfm_recv_rmep_list, node) {
						if(memcmp(entry->rmep_mac, mac, IFHWADDRLEN)==0) {
							attr.data = entry->eth_lm_single_rx_nloss;
							meinfo_me_attr_set(me, 4, &attr, 0);
							break;
						}
					}
				...
				if (util_attr_mask_get_bit(attr_mask, 5)) { // MinFeLoss
					...
					list_for_each_entry(entry, &cfm_config->cfm_recv_rmep_list, node) {
						if(memcmp(entry->rmep_mac, mac, IFHWADDRLEN)==0) {
							attr.data = (entry->eth_lm_single_rx_floss<entry->eth_lm_single_tx_floss)?entry->eth_lm_single_rx_floss:entry->eth_lm_single_tx_floss;
							meinfo_me_attr_set(me, 5, &attr, 0);
							break;
						}
					}
				...
				if (util_attr_mask_get_bit(attr_mask, 7)) { // AvgFeLoss
					...
					list_for_each_entry(entry, &cfm_config->cfm_recv_rmep_list, node) {
						if(memcmp(entry->rmep_mac, mac, IFHWADDRLEN)==0) {
							attr.data = (entry->eth_lm_single_rx_floss+entry->eth_lm_single_tx_floss)/2;
							meinfo_me_attr_set(me, 7, &attr, 0);
							break;
						}
					}
				...
				if (util_attr_mask_get_bit(attr_mask, 8)) { // CurFeLoss
					...
					list_for_each_entry(entry, &cfm_config->cfm_recv_rmep_list, node) {
						if(memcmp(entry->rmep_mac, mac, IFHWADDRLEN)==0) {
							attr.data = entry->eth_lm_single_rx_floss;
							meinfo_me_attr_set(me, 8, &attr, 0);
							break;
						}
					}
...
struct meinfo_callback_t meinfo_cb_calix_65302 = {
	.get_cb		= meinfo_cb_65302_get,
};
