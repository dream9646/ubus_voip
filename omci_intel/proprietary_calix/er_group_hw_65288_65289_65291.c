 ...
 * Module  : er_group_hw
 * File    : er_group_hw_65288_65289_65291.c
 *
 ******************************************************************/
...
//9.99.29 65291 OamLinkTrace

// 65288@3 [0]
// 65289@1,2,4,5 [1-4]
// 65291@1,2,3,4 [5-8]
int 
er_group_hw_oam_mep_linktrace(int action, struct er_attr_group_instance_t *attr_inst, struct me_t *me, unsigned char attr_mask[2])
{
	...
	case ER_ATTR_GROUP_HW_OP_ADD:
		if((cfm_config = cfm_mp_find(md_level, mepid, port_type, port_type_index)) != NULL) {
			unsigned short to_mepid = (unsigned short) attr_inst->er_attr_instance[5].attr_value.data;
			unsigned char ttl = (unsigned char) attr_inst->er_attr_instance[7].attr_value.data;
			unsigned char start = (unsigned char) attr_inst->er_attr_instance[8].attr_value.data;
			unsigned char to_mac[IFHWADDRLEN], null_mac[IFHWADDRLEN] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
			memcpy(to_mac, (unsigned char *) attr_inst->er_attr_instance[6].attr_value.ptr, sizeof(to_mac));
			...
			if(start) {
				cfm_send_ltm(cfm_config, to_mac, -1, 0, ttl, 1);
			}
		}
		break;
	...
	case ER_ATTR_GROUP_HW_OP_UPDATE:
		//delete
		if (er_group_hw_oam_mep_linktrace(ER_ATTR_GROUP_HW_OP_DEL, attr_inst, me, attr_mask) < 0)
		{
		...
		//add
		if (er_group_hw_oam_mep_linktrace(ER_ATTR_GROUP_HW_OP_ADD, current_attr_inst, me, attr_mask) < 0)
		{
