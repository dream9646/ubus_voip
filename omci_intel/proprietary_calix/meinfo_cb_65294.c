...
//classid 65294 9.99.32 OamMepExt

static int 
meinfo_cb_65294_get(struct me_t *me, unsigned char attr_mask[2])
{
	...
	if (util_attr_mask_get_bit(attr_mask, 2)) { // PmMacAddr
		...
		if((cfm_config = cfm_mp_find(md_level, mep_id, port_type, port_idx)) != NULL) {
			if (!cfm_config->admin_state) {
				if (util_attr_mask_get_bit(attr_mask, 2)) { // PmMacAddr
					struct attr_value_t attr;
					attr.ptr= malloc_safe(IFHWADDRLEN);
					memcpy(attr.ptr, cfm_config->cfm_send_dmm_mac, IFHWADDRLEN);
					meinfo_me_attr_set(me, 2, &attr, 0);
					if (attr.ptr) free_safe(attr.ptr);
				}
			}
...
struct meinfo_callback_t meinfo_cb_calix_65294 = {
	.get_cb		= meinfo_cb_65294_get,
};
