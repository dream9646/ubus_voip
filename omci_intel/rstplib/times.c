...
STP_get_times (IN BPDU_BODY_T *b, OUT TIMEVALUES_T *v)
{
  ...
  message_age = (unsigned short *)b->message_age;
  max_age = (unsigned short *)b->max_age;
  forward_delay = (unsigned short *)b->forward_delay;
  hello_time = (unsigned short *)b->hello_time;

  v->MessageAge =   ntohs (*message_age )   >> 8;
  v->MaxAge =       ntohs (*max_age )       >> 8;
  v->ForwardDelay = ntohs (*forward_delay ) >> 8;
  v->HelloTime =    ntohs (*hello_time )    >> 8;
}
